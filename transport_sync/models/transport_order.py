            ...
            for order_data in so_orders:
                try:
                    ...
                    if order_name:
                        ...
                        if existing:
                            skipped_count += 1
                            _logger.debug(f"⏭️ ข้าม: {order_name} - มีอยู่แล้ว")
                            continue
                    
                    # ✅ เช็คจาก odoo14_id ด้วย (กันซ้ำจาก ID)
                    existing_by_id = self.search([('odoo14_id', '=', order_data['id'])], limit=1)
                    if existing_by_id:
                        skipped_count += 1
                        _logger.debug(f"⏭️ ข้าม: ID {order_data['id']} - มีอยู่แล้ว")
                        continue
