...
class AccountWizard(models.TransientModel):
    ...
    section_report_ids = fields.Many2many(
        'account.report', 'wizard__axis_report_rel', 'wizard_id', 'report_id',
    )
    ...
    branch_id = fields.Many2one("res.branch",string="Branch",default=lambda self: self.env.user.branch_id,required=1)
    company_id = fields.Many2one("res.company",string="Company",default=lambda self: self.env.company,readonly=False)
    report_period = fields.Selection([('yearly','Yearly Report'),('datewise','DateWise Report')],default='yearly')
    previous_period =fields.Integer()
    def pdf_cash_flow_report(self):
        self.ensure_one()
        ...
        branch_nm = self.env['res.branch'].search([('id','=',int(self.branch_id))])
        company_nm = self.env['res.company'].search([('id','=',int(self.company_id))])
        data = {
            'ids': self.ids,
            'model': self._name,
            ...
            'branch_nm':branch_nm.name,
            'branch_id':branch_nm.id,
            'company_nm':company_nm.name,
            'report_period':self.report_period,
            'previous_period':self.previous_period
        }
        return self.env.ref(
            'multi_branch_management_aagam.pdf_report').report_action(self,
                                                                     data=data)

    def xlsx_cash_flow_report(self):
        ...
        return {
            ...
            'report_type': 'xlsx',
            'data': {'model': 'account.cash.flow.wizard',
                     'output_format': 'xlsx',
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('C3:G5', '')
        sheet.merge_range('C3:G4', 'CASH FLOW STATEMENTS', head)
        ...
        sheet.write('F6', "Branch Name", cell_format)
        sheet.write('G6', str(data['branch_nm']), date)
        sheet.write('F7', "Company Name", cell_format)
        sheet.write('G7', str(data['company_nm']), date)
